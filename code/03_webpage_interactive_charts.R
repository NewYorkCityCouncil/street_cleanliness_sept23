...
#
# Get interactive plots for webpage
################################################################################

# ------------------------------------------------------------------------------
# make the bar chart of oath violations vs 311 complaints
# ------------------------------------------------------------------------------
oath = fread(URLencode("https://data.cityofnewyork.us/resource/jz4z-kudi.csv?$limit=999999999999&$where=violation_date >= '2023-04-01' and violation_date < '2024-04-01'"))

x = oath %>%
  filter(violation_location_borough != "NOT NYC", 
         hearing_result != "DISMISSED", 
         charge_1_code_description %in% 
           c("DIRTY SIDEWALK", "DIRTY SIDEWALK DIRTY AREA  VACANT LOT", 
             "DIRTY SIDEWALK DIRTY AREA VACANT LOT", "DIRTY AREA", "ABANDONING VEHICLE") | 
           grepl("DUMPING", charge_1_code_description, ignore.case = T)) %>%
  select(charge_1_code_description) %>% 
  mutate(category = ifelse(grepl("DUMPING", charge_1_code_description), 
                           "Illegal Dumping", ""), 
         category = ifelse(grepl("VEHICLE", charge_1_code_description), 
                           "Derelict Vehicles", category), 
         category = ifelse(grepl("DIRTY", charge_1_code_description), 
                           "Dirty Condition", category)) %>% 
  group_by(category) %>% 
  summarise(n = n()) %>% 
  rename(complaint_type = category) %>%
  mutate(source = "OATH")
...
y = dsny_311 %>%
  filter(grepl("litter|dumping|derelict|dirty", complaint_type, ignore.case = T) |
           grepl("litter|dumping|derelict|dirty", descriptor, ignore.case = T), 
         complaint_type != "Litter Basket Request") %>% 
  mutate(complaint_type = ifelse(complaint_type == "Litter Basket Complaint", 
                                 "Dirty Condition", complaint_type)) %>%
...
plot = ggplot(comb, aes(x = reorder(complaint_type, n), y=n, fill = source)) + 
  geom_bar_interactive(stat = "identity", position = 'dodge',
                       width = 0.6,
                       tooltip = paste(comb$complaint_type, "\nTotal:", comb$n, 
                                       "\nType:", comb$source)) +
  coord_flip() +
  labs(x="",  y="Totals",
...
plot_interactive = girafe(ggobj = plot, width_svg = 6, height_svg = 5, 
                           options = list(opts_tooltip(css = "background-color:#CACACA;")))
...
dsny_311 = fread("https://data.cityofnewyork.us/resource/fhrw-4uyv.csv?agency='DSNY'&$where=created_date>='2022-04-01'&$limit=999999999999") %>%
  mutate(created_date = as.Date(created_date),
         date_ym = as.yearmon(created_date)) %>%
  filter(complaint_type %in% c("Overflowing Litter Baskets", "Dirty Condition", "Dirty", "Dirty Conditions", "Litter") |
           grepl("Litter", complaint_type, ignore.case = TRUE) |
           grepl("litter", descriptor, ignore.case = TRUE), 
         created_date < '2024-04-01')

dep_311 = fread("https://data.cityofnewyork.us/resource/fhrw-4uyv.csv?agency='DEP'&$where=created_date>='2022-04-01'&$limit=999999999999") %>%
  mutate(created_date = as.Date(created_date),
         date_ym = as.yearmon(created_date))

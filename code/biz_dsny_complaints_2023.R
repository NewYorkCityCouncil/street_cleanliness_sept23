...
rest_sub <-
  unique(rest[, .(latitude, longitude, bbl = as.character(bbl))])
...
biz_sub <-
  unique(biz[license_type %in% "Business" &
               license_status %in% "Active" & !location %in% "" &
...
business_complaints <-
  c("Commercial Disposal Complaint", "Retailer Complaint")
dsny_311_biz <-
  dsny_sub[complaint_type %in% business_complaints, .(location, unique_key)] %>%
...
biz_descs <-
  descs[grep(
    "commercial|food|restaurant|business|retail|drink|bar|beverage|
                        store|hotel|alcohol|chain|closing|organics",
    descs,
    ignore.case = TRUE
  )]
# remove dismissed
oath[charge_1_code_description %in% descs, mean(penalty_imposed)]
oath_sub <-
  oath[!hearing_result %in% "DISMISSED" &
         charge_1_code_description %in% biz_descs,
       .(
         boro = violation_location_borough,
...
pluto <-
  fread(
    "https://data.cityofnewyork.us/resource/64uk-42ks.csv?$limit=999999999&$select=bbl,latitude,longitude"
  )
pluto[, bbl := as.character(bbl)]
oathpl <- merge(oath_sub, pluto, by = "bbl")

oathsf <-
  oathpl[!is.na(latitude), .(longitude, latitude, unique_key)] %>%
